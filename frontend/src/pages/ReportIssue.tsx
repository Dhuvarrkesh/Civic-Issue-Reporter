...
const ReportIssue = () => {
  ...
  // Dev helper: allow setting Mapbox token at runtime (visible on localhost/non-prod)
  const envToken = import.meta.env.VITE_MAPBOXGL_ACCESS_TOKEN as string | undefined;
  const localToken = typeof window !== "undefined" ? localStorage.getItem("MAPBOX_TOKEN") : null;
  const hasToken = Boolean(envToken || localToken);
  const showDevTokenInput =
    (typeof window !== "undefined" && (window.location.hostname === "localhost" || window.location.hostname === "127.0.0.1")) ||
    (import.meta.env && (import.meta.env.MODE as string) !== "production");
  ...
  const saveToken = () => {
    ...
    toast.success("Mapbox token saved. Reloading to enable the map...");
  ...
  const handleAddressBlur = async (address: string) => {
    if (!address) return;
    // if we already have coordinates, don't geocode again
    if (formData.location.latitude && formData.location.longitude) return;

    const token = localToken || envToken;
    if (!token) {
      toast.error("Mapbox token not configured — please select location on map");
      return;
    }
    ...
    try {
      ...
      if (data.features && data.features[0]) {
        ...
        handleLocationSelect(lat, lng, place);
        toast.success("Location found and selected on map");
      } else {
        toast.error("Address not found — please select on map");
      }
    } catch (err) {
      console.error("Geocoding error:", err);
      toast.error("Failed to geocode address — try selecting on map");
    }
  };
  ...
  return (
    <div className="min-h-screen bg-[#f3f6f8]">
      {/* Header */}
      {showDevTokenInput && !hasToken && (
        <div className="bg-yellow-50 border border-yellow-200 p-3 text-sm text-yellow-800 text-center">
          <div className="max-w-xl mx-auto flex items-center gap-2">
            <input
              value={tokenInput}
              onChange={(e) => setTokenInput(e.target.value)}
              placeholder="Paste Mapbox token here to enable map"
          ...
          <div className="mt-2 text-xs text-muted-foreground">
            This is a dev helper — token is stored to localStorage as `MAPBOX_TOKEN` and the page will reload.
          </div>
        </div>
      )}
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      onBlur={() => handleAddressBlur(formData.issueLocation)}
                      placeholder="Enter or select location on map"
                      className="shadow-sm"
                    />
                    <p className="text-xs text-muted-foreground mt-1">
                      Tip: type an address and click away (blur) to auto-locate on the map — or select the exact point on the map.
                    </p>
                  </div>
