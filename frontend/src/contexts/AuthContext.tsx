...
export const AuthProvider: React.FC<AuthProviderProps> = ({ children }) => {
  ...
  const login = async (
    ...
    try {
      ...
      const body: any = { email: (email || "").toLowerCase(), password };
      ...
      if (role === "admin" && adminAccessCode) {
        body.adminAccessCode = Number(adminAccessCode);
      }
      ...
      // Save role and user id for fetch profile persistence
  ...
  const register = async (userData: any, role: "citizen" | "admin") => {
    ...
    try {
      ...
      // Normalize email
      if (userData && userData.email) userData.email = userData.email.toLowerCase();
